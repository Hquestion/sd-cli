...
module.exports = {
    async register(name, cmd) {
        ...
        if (cmd.path) {
            ...
            if (fs.existsSync(cfgPath)) {
                console.log(chalk.yellow(`使用${cmdPath}下的脚本注册聚合接口，请确保脚本位置正确！`));
                scriptPath = cmdPath;
        ...
        execa.commandSync('chmod 777 ./register-tool', {
    ...
    async unregister(name, cmd) {
        ...
        if (cmd.path) {
            ...
            let cfgPath = path.resolve(cmdPath, 'config.js');
            if (fs.existsSync(cfgPath)) {
                console.log(chalk.yellow(`使用${cmdPath}下的脚本删除聚合接口，请确保脚本位置正确！`));
